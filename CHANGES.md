## Current
* core.event
 - Add SparseEvent classes as drop-in replacements for ObsPy event objects.
   The aim is to have a lighter class for handling the minimal event information
   required and generated by EQcorrscan, with a known json representation for
   fast IO. These classes can be converted to obspy Event classes fairly simply
   to allow interoperability - however, not all attributes and only a few methods
   are retained in the SparseXXX classes.   
